...
- **브라우저 보조 흐름**: 로컬 Playwright 세션으로 네이버 land 페이지를 직접 열고 same-origin fetch/capture helper를 써 403/429 환경에서 direct URL/ID를 확보하거나 세션 기반 조회를 보조한다.
...
2. direct 단서가 애매하면 `search_real_estate.py --resolve-direct` 또는 `--lookup-complex`로 먼저 정리한다.
3. 403/429가 걸리거나 URL/ID 확보가 필요하면 `browser_session_helper.py capture/fetch`로 local browser 세션 보조 흐름을 쓴다.
4. URL/ID가 없으면 `chat_real_estate.py --query ...` 로 먼저 자연어 브리핑을 시도한다.
5. 질의가 넓거나 모호하면 `--list-candidates` 로 후보를 1~3개만 보여준다.
6. 후보가 좁혀지면 단일 단지 조회 또는 `--compare` 로 비교한다.
7. 구조화 데이터가 필요하면 같은 조건으로 `search_real_estate.py --json` 을 다시 호출한다.
8. 반복 확인이 필요하면 `watch_real_estate.py add/check` 흐름으로 가격 감시를 연결한다.
